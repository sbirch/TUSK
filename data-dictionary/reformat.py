...
for variable in order:
	...
	if entry['validEntries'] is not None:
		...
		values = '<ul class="valid-entries">%s</ul>' % ''.join(entries)
	...
	content.append('''<hr /><div class="variable">
		...
		', '.join(entry['files']),
		values,
		'' if entry['note'] == None else '<div class="note"><span>Note: </span>%s</div>' % re.sub('(\s|\(|\[)([GHPT][UERT][A-Z0-9_]{1,12})', r'\1<a class="crossref" href="#\2">\2</a>', entry['note'])
		)
